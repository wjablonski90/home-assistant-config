...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_RETRY_COUNT, default=5): cv.positive_int,
...
class Host:
    ...
    def __init__(self, ip_address, dev_id, hass, config):
        ...
        self._retries = config[CONF_RETRY_COUNT]
        self._iface = config[CONF_IFACE]

        if self._iface:
            self._ping_cmd = ['ping', '-I', self._iface, '-n', '-q', '-c', str(self._count), '-W1', self.ip_address]
            self._parp_cmd = ['arp', '-i', self._iface, '-n', self.ip_address]
        else:
            self._ping_cmd = ['ping', '-n', '-q', '-c', str(self._count), '-W1', self.ip_address]
    ...
    def update(self, see):
        ...
        while failed < self._retries:  # check more times if host is unreachable
